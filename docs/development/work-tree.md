...
| Reporting and Operational Analytics | In Progress | FO-064 + FO-064A + FO-065 + FO-065A complete on `feature/reporting` (draft PR #38); `/reporting` overview uses `reporting.view` filter-options endpoint; status/priority UX (FO-066), export, and charts deferred |
