...
| FM Ticketing | Complete | Backend workflows plus frontend read, create, edit, comments, assignment (FO-048 directory picker via FO-061A), SLA, escalation; FO-061/FO-061A Generate Work Order into linked Maintenance with active assignment (FO-061B approved foundation; FO-062 status sync pending) |
| Maintenance Work Order | Complete | FO-031 through FO-037 complete; FO-061 adds nullable OneToOne `source_ticket` and source-ticket detail display; FO-061A routes generated WOs through `assign_work_order()`; FO-061B validates foundation for FO-062 |
| FM Ticket ↔ Maintenance Integration | In Progress | FO-061/FO-061A cumulatively approved (FO-061B 2026-07-14). Draft PR #36 open/draft/unmerged. Next: FO-062 status synchronization |
...
| Testing | Complete | FO-061B (2026-07-14): Backend `apps.fm_tickets` 63, `apps.maintenance` 69, `apps.notifications` 78, `apps.accounts`+`access_control` 109, full `--parallel 4` 411 OK; Frontend: 122 helper tests, ESLint, TypeScript, and production build pass; no component/integration harness exists yet |
