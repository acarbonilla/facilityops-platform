...
import { useCreateProjectTaskComment } from "@/hooks/use-projects";
import {
  formatPersonLabel,
  formatProjectDateTime,
} from "@/lib/projects/display";
import { formatProjectTaskError } from "@/lib/projects/tasks-display";
import type { ProjectTaskComment } from "@/types/projects";
...
export function ProjectTaskComments({
  ...
  const createMutation = useCreateProjectTaskComment(projectId, taskId);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setErrorMessage(
        formatProjectTaskError(error, "Comment could not be created."),
      );
    }
  }
    ...
    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {comments.length === 0 ? (
        <EmptyState
          ...
          message="Add a comment to leave coordination notes on this task."
        />
      ) : (
      ...
      {canComment ? (
        <div className="space-y-3">
          <FormField label="New comment" htmlFor="task-comment-body">
            <textarea
